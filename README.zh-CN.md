...
![Solo 系统架构](docs/architecture/solo-system-architecture.png)

> 更高分辨率的 SVG 版本见 [`docs/architecture/solo-system-architecture.svg`](docs/architecture/solo-system-architecture.svg)。
...
| **JS** | push/PR 到 main | `npm ci` → lint (app, app-bridge, highlight) → typecheck → test (app 1282 测试, app-bridge 32 测试) → Codecov 上传 |
| **E2E** (每日) | 每日 02:00 UTC + 手动 | Playwright E2E (22 个测试规格) 含 daemon/relay/Metro globalSetup |
