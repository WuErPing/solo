...
| 前端 | Expo 57 · React Native 0.86 · React 19 · TypeScript |
...
- **日程助手** — 在对话面板中用自然语言创建和编辑定时任务（"Ask AI"），由主机配置的 LLM 提供商驱动；所有变更均为提案，经用户明确确认后才会应用
...
- **循环模板** — 可复用的模板 CRUD 用于定义循环配置；实例按模板分组
- **应用界面** — 侧边栏中专用的循环列表、详情、实例详情和创建页面
- **CLI 命令** — `solo-cli loop ls|run|status|stop|update|delete`
- **提供商集成** — 循环使用现有 AI 提供商执行迭代步骤
- **执行跟踪** — 完整的运行历史、日志文件分离和状态监控
