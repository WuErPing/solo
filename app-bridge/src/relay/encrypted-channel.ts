  ...
  return new Promise((resolve, reject) => {
    ...
    const handleHello = async (data: string | ArrayBuffer): Promise<void> => {
      try {
        ...
        transport.send(JSON.stringify({ type: "e2ee_ready" } as E2EEReadyMessage));
...
export class EncryptedChannel {
  ...
  private async handleDaemonRehello(clientKeyB64: string): Promise<void> {
    ...
    // If it's the same client key (handshake retry), re-send "ready" but do not re-key.
    if (keysEqual(nextSharedKey, this.sharedKey)) {
      this.transport.send(JSON.stringify({ type: "e2ee_ready" } as E2EEReadyMessage));
      return;
    }

    // Different key implies a new client connection. Re-key and re-send "ready".
