...
export function isSoloToolName(name: string): boolean {
  const normalized = normalizeToolName(name);
  ...
  if (normalized.includes("__")) {
    ...
    return (
      segments.length >= 3 &&
      segments[0] === "mcp" &&
      (segments[1] === "solo" || segments[1]!.startsWith("solo_"))
    );
  }
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0]!;
    return firstSegment === "solo" || firstSegment.startsWith("solo_");
  }
  return false;
}

export function getSoloToolLeafName(name: string): string | null {
  const normalized = normalizeToolName(name);
  if (normalized.includes("__")) {
    const segments = normalized.split("__").filter((s) => s.length > 0);
    if (
  ...
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0]!;
    if (firstSegment === "solo" || firstSegment.startsWith("solo_")) {
