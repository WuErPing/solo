...
test.beforeEach(async ({ page }) => {
  ...
  await page.addInitScript(
    ({ daemon, preferences, seedNonce: nonce }) => {
      ...
      const disableOnceKey = "@solo:e2e-disable-default-seed-once";
      ...
      localStorage.setItem("@solo:e2e-seed-nonce", nonce);

      // Hard-reset anything that could point to a developer's real daemon.
      localStorage.setItem("@solo:daemon-registry", JSON.stringify([daemon]));
      localStorage.removeItem("@solo:settings");
      localStorage.setItem("@solo:create-agent-preferences", JSON.stringify(preferences));
    },
