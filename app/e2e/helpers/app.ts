...
async function ensureE2EStorageSeeded(page: Page): Promise<void> {
  ...
  const needsReset = await page.evaluate(
    ({ expectedEndpoint: endpoint, expectedServerId: serverId }) => {
      const raw = localStorage.getItem("@solo:daemon-registry");
      if (!raw) return true;
      try {
...
function parseRegistryEntry(registryRaw: string): { serverId: string; connections: unknown } {
  ...
  try {
    ...
    throw new Error("E2E expected @solo:daemon-registry to be valid JSON.");
  }
  if (!Array.isArray(registry) || registry.length !== 1) {
    throw new Error(
      `E2E expected @solo:daemon-registry to contain exactly 1 daemon (got ${Array.isArray(registry) ? registry.length : "non-array"}).`,
    );
  }
...
function assertPreferencesMatch(prefsRaw: string, serverId: string): void {
  try {
    ...
    throw new Error("E2E expected @solo:create-agent-preferences to be valid JSON.", {
      cause: error,
    });
  }
}
...
async function assertE2EUsesSeededTestDaemon(page: Page): Promise<void> {
  ...
  const snapshot = await page.evaluate(() => {
    const registryRaw = localStorage.getItem("@solo:daemon-registry");
    const prefsRaw = localStorage.getItem("@solo:create-agent-preferences");
    return { registryRaw, prefsRaw };
  });

  if (!snapshot.registryRaw) {
    throw new Error("E2E expected @solo:daemon-registry to be set before app load.");
  }
  ...
  if (!snapshot.prefsRaw) {
    throw new Error("E2E expected @solo:create-agent-preferences to be set before app load.");
  }
...
export const ensureHostSelected = async (page: Page) => {
  ...
  try {
    ...
    const fix = await page.evaluate(() => {
      ...
      localStorage.setItem("@solo:create-agent-preferences", JSON.stringify(prefs));
      // Prevent the fixture's init-script from overwriting the corrected prefs on reload.
      const nonce = localStorage.getItem("@solo:e2e-seed-nonce") ?? "1";
      localStorage.setItem("@solo:e2e-disable-default-seed-once", nonce);
