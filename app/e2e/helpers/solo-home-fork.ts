...
export interface SoloHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveSoloHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkSoloHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<SoloHomeMetadataForkResult> {
  const sourceHome = resolveSoloHomePath(input.sourceHome);
  const targetHome = resolveSoloHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Solo metadata onto the same SOLO_HOME.");
  }
