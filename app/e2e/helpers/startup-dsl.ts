...
const DISABLE_DEFAULT_SEED_ONCE_KEY = "@solo:e2e-disable-default-seed-once";
const SEED_NONCE_KEY = "@solo:e2e-seed-nonce";
const REGISTRY_KEY = "@solo:daemon-registry";
const E2E_KEY = "@solo:e2e";
  ...
  private async prepare(): Promise<void> {
    ...
    await this.page.evaluate(
      ({ keys, registry: storedRegistry, createAgentPreferences: storedPreferences }) => {
        ...
        localStorage.setItem("@solo:create-agent-preferences", JSON.stringify(storedPreferences));
...
class StartupAssertions {
  ...
  async expectsNoLocalServerStartupCopy(): Promise<this> {
    await expect(this.page.getByTestId("startup-splash-container")).toHaveCount(0);
    return this;
  }

  async expectsDesktopDaemonStartup(): Promise<this> {
    await expect(this.page.getByTestId("startup-splash-container")).toBeVisible({
      timeout: 15_000,
    });
    return this;
  }
...
async function installPendingDesktopBridge(page: Page): Promise<void> {
  await page.addInitScript(() => {
    (window as unknown as { soloDesktop: unknown }).soloDesktop = {
      platform: "darwin",
