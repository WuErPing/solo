...
export async function installTerminalRenderProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    interface ProbeTerm {
      ...
      __soloRenderProbeWriteWrapped?: boolean;
    }
    ...
    const win = window as unknown as Record<string, unknown> & {
      ...
      __soloTerminal?: ProbeTerm;
    };
    const existingDescriptor = Object.getOwnPropertyDescriptor(win, "__soloTerminal");
    const getExisting = () =>
    ...
    Object.defineProperty(win, "__soloTerminal", {
      configurable: true,
      get() {
        return probe.term;
      },
      set(next: ProbeTerm | undefined) {
        ...
        if (next?.write && !next.__soloRenderProbeWriteWrapped) {
          ...
          next.__soloRenderProbeWriteWrapped = true;
        }
      },
    });
  });
...
export async function installTerminalKeystrokeStressProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    const existingDescriptor = Object.getOwnPropertyDescriptor(window, "__soloTerminal");
    const getExisting = () =>
      existingDescriptor?.get ? existingDescriptor.get.call(window) : existingDescriptor?.value;

    let terminal = getExisting();
    Object.defineProperty(window, "__soloTerminal", {
      configurable: true,
      get() {
        return terminal;
      },
      set(next: {
        write?: (data: string | Uint8Array, callback?: () => void) => void;
        __soloKeystrokeProbeWriteWrapped?: boolean;
      }) {
        terminal = next;
        if (next?.write && !next.__soloKeystrokeProbeWriteWrapped) {
          const originalWrite = next.write.bind(next);
          next.write = (data: string | Uint8Array, callback?: () => void) => {
            const text =
              typeof data === "string" ? data : new TextDecoder().decode(data as Uint8Array);
          ...
          next.__soloKeystrokeProbeWriteWrapped = true;
        }
      },
    });
  });
