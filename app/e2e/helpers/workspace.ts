...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@solo.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "Solo E2E"', { cwd: repoPath, stdio: "ignore" });
  execSync("git config commit.gpgsign false", { cwd: repoPath, stdio: "ignore" });
  await writeFile(path.join(repoPath, "README.md"), "# Temp Repo\n");
  if (options?.soloConfig) {
    await writeFile(
      path.join(repoPath, "solo.json"),
      JSON.stringify(options.soloConfig, null, 2),
    );
  }
  ...
  if (options?.soloConfig) {
    execSync("git add solo.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.solo-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
