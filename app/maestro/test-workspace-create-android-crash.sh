...
OUT_DIR="/tmp/solo-workspace-create-android-$(date +%s)"
SERVER_EXPORTS="$REPO_ROOT/packages/server/dist/server/server/exports.js"

export SOLO_MAESTRO_APP_ID="${SOLO_MAESTRO_APP_ID:-sh.solo.debug}"
export SOLO_MAESTRO_DIRECT_ENDPOINT="${SOLO_MAESTRO_DIRECT_ENDPOINT:-127.0.0.1:6767}"
export SOLO_MAESTRO_DAEMON_WS_URL="${SOLO_MAESTRO_DAEMON_WS_URL:-ws://127.0.0.1:6767/ws}"
...
render_flow() {
  ...
  perl -0pe '
    s/\$\{SOLO_MAESTRO_APP_ID\}/$ENV{SOLO_MAESTRO_APP_ID}/g;
    s/\$\{SOLO_MAESTRO_DIRECT_ENDPOINT\}/$ENV{SOLO_MAESTRO_DIRECT_ENDPOINT}/g;
    s/\$\{SOLO_MAESTRO_PROJECT_NAME\}/$ENV{SOLO_MAESTRO_PROJECT_NAME}/g;
  ' "$source" > "$target"
}
...
if [ ! -f "$SERVER_EXPORTS" ]; then
  ...
  echo "Run: npm run build --workspace=@getsolo/server" >&2
  exit 1
fi

mkdir -p "$OUT_DIR"

if [ -z "${SOLO_MAESTRO_PROJECT_PATH:-}" ]; then
  PROJECT_PARENT="$(mktemp -d /tmp/solo-maestro-project-XXXXXX)"
  PROJECT_BASENAME="aaa-workspace-create-android-$(basename "$PROJECT_PARENT")"
  export SOLO_MAESTRO_PROJECT_PATH="$PROJECT_PARENT/$PROJECT_BASENAME"
  mkdir -p "$SOLO_MAESTRO_PROJECT_PATH"
  git -C "$SOLO_MAESTRO_PROJECT_PATH" init >/dev/null
  git -C "$SOLO_MAESTRO_PROJECT_PATH" checkout -b main >/dev/null 2>&1 || true
  git -C "$SOLO_MAESTRO_PROJECT_PATH" config user.name "Solo Maestro"
  git -C "$SOLO_MAESTRO_PROJECT_PATH" config user.email "maestro@getsolo.local"
  printf "# Workspace create Android repro\n" > "$SOLO_MAESTRO_PROJECT_PATH/README.md"
  git -C "$SOLO_MAESTRO_PROJECT_PATH" add README.md
  git -C "$SOLO_MAESTRO_PROJECT_PATH" commit -m "Initial commit" >/dev/null
else
  PROJECT_PARENT=""
fi

export SOLO_MAESTRO_PROJECT_NAME="${SOLO_MAESTRO_PROJECT_NAME:-$(basename "$SOLO_MAESTRO_PROJECT_PATH")}"

echo "=== Workspace Create Android Crash Harness ==="
echo "Output dir: $OUT_DIR"
echo "App id: $SOLO_MAESTRO_APP_ID"
echo "Android direct endpoint: $SOLO_MAESTRO_DIRECT_ENDPOINT"
echo "Daemon websocket: $SOLO_MAESTRO_DAEMON_WS_URL"
echo "Project: $SOLO_MAESTRO_PROJECT_PATH"
echo "Project name: $SOLO_MAESTRO_PROJECT_NAME"
...
const projectPath = process.env.SOLO_MAESTRO_PROJECT_PATH;
const daemonUrl = process.env.SOLO_MAESTRO_DAEMON_WS_URL;
