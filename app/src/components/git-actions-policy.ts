...
export function buildGitActions(input: BuildGitActionsInput): GitActions {
  ...
  allActions.set("archive-worktree", {
    ...
    unavailableMessage:
      input.runtime["archive-worktree"].disabled || input.isSoloOwnedWorktree
        ? undefined
        : "Archive isn't available here because this workspace was not created as a Solo worktree",
...
function getPrimaryActionId(input: BuildGitActionsInput): GitActionId | null {
  if (input.shouldPromoteArchive && input.isSoloOwnedWorktree) {
