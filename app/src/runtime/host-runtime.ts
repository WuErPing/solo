...
const REGISTRY_STORAGE_KEY = "@solo:daemon-registry";
const DEFAULT_LOCALHOST_ENDPOINT = process.env.EXPO_PUBLIC_LOCAL_DAEMON?.trim() || "localhost:17612";
const ENV_RELAY_ENDPOINT = process.env.EXPO_PUBLIC_RELAY_ENDPOINT?.trim() || "";
const ENV_RELAY_SERVER_ID = process.env.EXPO_PUBLIC_RELAY_SERVER_ID?.trim() || "";
const ENV_RELAY_PUBLIC_KEY = process.env.EXPO_PUBLIC_RELAY_PUBLIC_KEY?.trim() || "";
...
const E2E_STORAGE_KEY = "@solo:e2e";
...
export class HostRuntimeStore {
  ...
  async bootstrap(options?: { manageBuiltInDaemon?: boolean }): Promise<void> {
    ...
    await this.bootstrapRelayFromEnv();
  }
  ...
  private async bootstrapRelayFromEnv(): Promise<void> {
    if (!ENV_RELAY_ENDPOINT || !ENV_RELAY_SERVER_ID || !ENV_RELAY_PUBLIC_KEY) {
      return;
    }
    try {
      const relayEndpoint = normalizeHostPort(ENV_RELAY_ENDPOINT);
      const connection: HostConnection = {
        id: `relay:${relayEndpoint}`,
        type: "relay",
        relayEndpoint,
        daemonPublicKeyB64: ENV_RELAY_PUBLIC_KEY,
      };
      if (registryHasConnection(this.hosts, connection)) {
        return;
      }
      await this.upsertHostConnection({
        serverId: ENV_RELAY_SERVER_ID,
        connection,
      });
    } catch (error) {
      console.warn("[HostRuntime] Failed to bootstrap relay from env", error);
    }
  }
...
const HOST_RUNTIME_STORE_GLOBAL_KEY = "__soloHostRuntimeStore";
