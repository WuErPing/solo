...
import type { CreateSoloWorktreeInput } from "@server/client/daemon-client";
...
const {
  ...
  const hoistedCreatedWorkspace = {
    id: "workspace-1",
    workspaceDirectory: "/repo/.solo/worktrees/workspace-1",
  };
  ...
  const hoistedMockClient = {
    ...
    createSoloWorktree: vi.fn(async (_input: CreateSoloWorktreeInput) => ({
...
beforeEach(() => {
  ...
  mockClient.createSoloWorktree.mockClear();
...
type CreateSoloWorktreeArg = Parameters<typeof mockClient.createSoloWorktree>[0];
...
function firstCreateWorktreeCall(): CreateSoloWorktreeArg {
  const calls = mockClient.createSoloWorktree.mock.calls;
  const firstCall = calls[0];
  if (!firstCall) throw new Error("createSoloWorktree not called");
  return firstCall[0];
}
...
describe("NewWorkspaceScreen picker payload", () => {
  ...
  it("sends no new fields when nothing is selected (default)", async () => {
    ...
    expect(mockClient.createSoloWorktree).toHaveBeenCalledTimes(1);
  ...
  it("preserves and locks the composer and picker while worktree creation is pending, then unlocks on error", async () => {
    ...
    mockClient.createSoloWorktree.mockImplementationOnce(async () => await createWorktree.promise);
    renderScreen();
    await flush();
