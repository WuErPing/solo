...
type ReadResult =
  | { ok: true; config: SoloConfigRaw | null; revision: SoloConfigRevision | null }
  | { ok: false; error: ProjectConfigRpcError };
type WriteResult =
  | { ok: true; config: SoloConfigRaw; revision: SoloConfigRevision }
  | { ok: false; error: ProjectConfigRpcError };
...
function readSuccess(input: {
  config: SoloConfigRaw | null;
  revision: SoloConfigRevision | null;
}): ReadResult {
...
describe("ProjectSettingsScreen — save flow", () => {
  ...
  it("issues writeProjectConfig with the current config + expectedRevision and installs the response", async () => {
    ...
    const baseConfig: SoloConfigRaw = {
      worktree: { setup: "npm install" },
      customTopLevel: "preserved",
    } as unknown as SoloConfigRaw;
    const initialRevision: SoloConfigRevision = { mtimeMs: 100, size: 50 };
    const newRevision: SoloConfigRevision = { mtimeMs: 200, size: 60 };

    client.readProjectConfig.mockResolvedValue(
      readSuccess({ config: baseConfig, revision: initialRevision }),
    );
    client.writeProjectConfig.mockImplementation(
      async (input: { config: SoloConfigRaw; expectedRevision: SoloConfigRevision | null }) =>
    ...
    const secondArg = client.writeProjectConfig.mock.calls[1]![0] as {
      expectedRevision: SoloConfigRevision;
    };
  ...
  it("renders the invalid_project_config callout with a Reload action", async () => {
    ...
    expect(callout.textContent).toContain("solo.json couldn't be parsed");
  ...
  it("renders the write_failed callout with Try again and Reload, and Try again retries the save", async () => {
    ...
    expect(callout.textContent).toContain("Couldn't save solo.json");
...
describe("ProjectSettingsScreen — round-trip semantics", () => {
  it("preserves the original lifecycle string kind on in-place edits", async () => {
    ...
    client.writeProjectConfig.mockImplementation(async (input: { config: SoloConfigRaw }) =>
    ...
    const savedConfig = client.writeProjectConfig.mock.calls[0]![0]!.config as SoloConfigRaw;
