...
function ProjectSettingsBody({
  ...
  const loadedConfig: SoloConfigRaw | null = data?.ok ? (data.config ?? {}) : null;
  const loadedRevision: SoloConfigRevision | null = data?.ok ? data.revision : null;
...
interface RenderContentInput {
  ...
  loadedConfig: SoloConfigRaw | null;
  loadedRevision: SoloConfigRevision | null;
...
function revisionToKey(revision: SoloConfigRevision | null): string {
...
function resolveReadFailureCopy(input: {
  ...
  if (input.kind === "invalid_project_config") {
    return {
      ...
      title: "solo.json couldn't be parsed",
      ...
      {isStale ? (
        <View style={styles.calloutWrap}>
          <Alert
            ...
            description="Reload to fetch the latest solo.json before saving."
          >
            <Button
        ...
        <View style={styles.serviceToggleRow}>
          <View style={styles.serviceToggleText}>
            ...
            <Text style={styles.modalHint}>
              Solo supervises the process and assigns a port via $SOLO_PORT
            </Text>
          </View>
          <Switch
            value={isService}
