...
function invalidateWorktreeList() {
  void appQueryClient.invalidateQueries({
    predicate: (query) =>
      Array.isArray(query.queryKey) && query.queryKey[0] === "soloWorktreeList",
  });
  void appQueryClient.invalidateQueries({
    predicate: (query) =>
      Array.isArray(query.queryKey) && query.queryKey[0] === "sidebarSoloWorktreeList",
  });
}
  ...
  appQueryClient.setQueriesData(
    {
      predicate: (query) =>
        ...
        query.queryKey[0] === "soloWorktreeList" &&
  ...
  appQueryClient.setQueriesData(
    {
      predicate: (query) =>
        Array.isArray(query.queryKey) &&
        query.queryKey[0] === "sidebarSoloWorktreeList" &&
        query.queryKey[1] === serverId,
    },
    removeFromList,
  );
...
function isWorktreeListQuery(input: { queryKey: QueryKey; serverId: string }): boolean {
  return (
    ...
    (input.queryKey[0] === "soloWorktreeList" ||
      input.queryKey[0] === "sidebarSoloWorktreeList") &&
  ...
  archiveWorktree: async ({ serverId, cwd, worktreePath }) => {
    await runCheckoutAction({
      ...
      run: async () => {
        ...
        try {
          const payload = await client.archiveSoloWorktree({ worktreePath });
          if (payload.error) {
            throw new Error(payload.error.message);
          }
        } catch (error) {
