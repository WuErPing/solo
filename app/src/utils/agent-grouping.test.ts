  ...
  it("shows owner/repo for GitHub remote keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "remote:github.com/getsolo/solo",
        projectName: "solo",
      }),
    ).toBe("getsolo/solo");
  });
...
describe("groupAgents", () => {
  it("groups active agents by remote URL when available", () => {
    const agents = [
      makeAgent({ id: "a1", cwd: "/Users/me/dev/solo" }),
      makeAgent({ id: "a2", cwd: "/Users/me/dev/solo-fix/worktree" }),
    ];

    const { activeGroups } = groupAgents(agents, {
      getRemoteUrl: () => "git@github.com:getsolo/solo.git",
    });
