...
const SOLO_WORKTREE_PATH_PATTERN = /(^|\/)\.solo\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isSoloOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isSoloOwnedWorktreePath(cwd: string): boolean {
  return SOLO_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
