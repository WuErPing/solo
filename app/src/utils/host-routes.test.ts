...
describe("workspace route parsing", () => {
  ...
  it("decodes non-canonical base64url workspace IDs used by older links", () => {
    expect(decodeWorkspaceIdFromPathSegment("L1VzZXJzL21vYm91ZHJhL2Rldi9zb2xv")).toBe(
      "/Users/moboudra/dev/solo",
    );
  });
...
describe("projects settings routes", () => {
  ...
  it("buildProjectSettingsRoute encodes a local repo-root key", () => {
    expect(buildProjectSettingsRoute("/Users/me/dev/solo")).toBe(
      "/settings/projects/%2FUsers%2Fme%2Fdev%2Fsolo",
    );
  });
