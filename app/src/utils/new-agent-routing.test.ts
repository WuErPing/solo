...
describe("buildNewAgentRoute", () => {
  ...
  it("encodes the working directory as a workspace path segment", () => {
    expect(buildNewAgentRoute("srv-1", "/Users/me/dev/solo")).toBe(
      "/h/srv-1/workspace/b64_L1VzZXJzL21lL2Rldi9zb2xv",
    );
  });
});
...
describe("resolveNewAgentWorkingDir", () => {
  ...
  it("falls back to repo root when checkout metadata is unavailable", () => {
    expect(resolveNewAgentWorkingDir("/repo/.solo/worktrees/feature", null)).toBe("/repo");
  });

  it("supports windows-style solo worktree paths without checkout metadata", () => {
    expect(resolveNewAgentWorkingDir("C:\\Users\\me\\repo\\.solo\\worktrees\\feature", null)).toBe(
      "C:\\Users\\me\\repo",
    );
  });

  it("returns the main repo root for solo-owned worktrees", () => {
    const checkout = {
      isSoloOwnedWorktree: true,
      worktreeRoot: "/repo/.solo/worktrees/feature",
      mainRepoRoot: "/repo/main",
    } as unknown as CheckoutStatusPayload;

    expect(resolveNewAgentWorkingDir("/repo/.solo/worktrees/feature", checkout)).toBe(
      "/repo/main",
    );
  });
});
