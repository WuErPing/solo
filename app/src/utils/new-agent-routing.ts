...
function inferMainRepoRootFromSoloWorktreePath(cwd: string): string | null {
  const normalizedPath = cwd.replace(/\\/g, "/");
  const marker = "/.solo/worktrees";
...
export function resolveNewAgentWorkingDir(
  ...
  const explicitMainRepoRoot = checkout?.isSoloOwnedWorktree
  ...
  return inferMainRepoRootFromSoloWorktreePath(cwd) ?? cwd;
}
