import type { SoloConfigRaw, SoloScriptEntryRaw } from "@server/shared/messages";
...
export function configToDraft(config: SoloConfigRaw | null | undefined): ProjectConfigDraft {
...
interface ApplyDraftInput {
  ...
  base: SoloConfigRaw | null | undefined;
}

export function applyDraftToConfig(input: ApplyDraftInput): SoloConfigRaw {
  ...
  const nextScripts: Record<string, SoloScriptEntryRaw> = {};
  for (const row of input.draft.scripts) {
    ...
    nextScripts[trimmedName] = nextEntry as SoloScriptEntryRaw;
  }
  ...
  return result as SoloConfigRaw;
}
