...
describe("project-placement", () => {
  ...
  it("normalizes solo worktree paths into the parent repo key", () => {
    const placement = deriveProjectPlacementFromCwd("/Users/test/repo/.solo/worktrees/feature-x");

    expect(placement.projectKey).toBe("/Users/test/repo");
    expect(placement.projectName).toBe("repo");
    expect(placement.checkout.cwd).toBe("/Users/test/repo/.solo/worktrees/feature-x");
  });
