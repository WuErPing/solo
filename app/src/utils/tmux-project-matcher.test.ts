...
describe("matchTmuxToProjects", () => {
  ...
  it("matches exact path", () => {
    ...
    expect(result.get("p1")).toEqual({ agentCount: 0, paneCount: 1 });
  });
  ...
  it("counts multiple panes for the same project", () => {
    ...
    expect(result.get("p1")).toEqual({ agentCount: 0, paneCount: 3 });
  });
  ...
  it("matches panes to different projects independently", () => {
    ...
    expect(result.get("pA")).toEqual({ agentCount: 0, paneCount: 1 });
    expect(result.get("pB")).toEqual({ agentCount: 0, paneCount: 1 });
  });
  ...
  it("distinguishes agent panes from regular panes", () => {
    ...
    expect(result.get("p1")).toEqual({ agentCount: 2, paneCount: 3 });
  });
