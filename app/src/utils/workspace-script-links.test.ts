...
const runningService: WorkspaceScriptPayload = {
  scriptName: "web",
  type: "service",
  hostname: "web.feature.solo.localhost",
  port: 3000,
  proxyUrl: "http://web.feature.solo.localhost:6767",
  lifecycle: "running",
  health: "healthy",
  exitCode: null,
  terminalId: null,
...
describe("resolveWorkspaceScriptLink", () => {
  it("uses the local proxy URL for loopback TCP connections", () => {
    expect(
      ...
      openUrl: "http://web.feature.solo.localhost:6767",
      labelUrl: "http://web.feature.solo.localhost:6767",
    });
  });

  it("uses the local proxy URL for socket and pipe connections", () => {
    expect(
      resolveLink({ type: "directSocket", endpoint: "/tmp/solo.sock", display: "socket" }),
    ).toEqual({
      openUrl: "http://web.feature.solo.localhost:6767",
      labelUrl: "http://web.feature.solo.localhost:6767",
    });
  ...
  it("shows the local proxy URL but disables opening over relay", () => {
    expect(
      resolveLink({ type: "relay", endpoint: "relay.solo.sh:443", display: "relay" }),
    ).toEqual({
      openUrl: null,
      labelUrl: "http://web.feature.solo.localhost:6767",
    });
