...
const ls = w.localStorage ?? (w.window as Record<string, unknown> | undefined)?.localStorage;
if (typeof (ls as Record<string, unknown> | undefined)?.setItem !== "function") {
  w.localStorage = localStorageShim();
  if (w.window) {
    (w.window as Record<string, unknown>).localStorage = w.localStorage;
  }
}
