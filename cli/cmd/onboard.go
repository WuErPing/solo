...
func runOnboard(_ *cobra.Command, _ []string) error {
	_, _ = fmt.Fprintln(cmdStdout, output.Bold("Welcome to Solo!"))
	...
	_, _ = fmt.Fprintf(cmdStdout, "Solo home: %s\n", home)
	...
	if _, err := client.LoadOrCreateDaemonKeyPair(); err != nil {
		_, _ = fmt.Fprintf(cmdStdout, output.Yellow("Warning: could not verify daemon keypair: %v\n"), err)
	}

	// Step 1: Start daemon if not running
	running, pid, _ := client.IsDaemonRunning()
	if running {
		_, _ = fmt.Fprintf(cmdStdout, "Daemon already running (PID %d)\n", pid)
	} else {
		_, _ = fmt.Fprintln(cmdStdout, "Starting daemon...")
	...
	_, _ = fmt.Fprintln(cmdStdout, "Waiting for daemon to become ready...")
	...
	_, _ = fmt.Fprintf(cmdStdout, "Daemon ready on %s\n", host)
	...
	if relayDisabled {
		_, _ = fmt.Fprintln(cmdStdout, output.Yellow("Relay is disabled; pairing is unavailable."))
		printNextSteps(home, "")
		return nil
	}
	...
	if err != nil {
		_, _ = fmt.Fprintln(cmdStdout, output.Yellow("Pairing offer unavailable."))
		printNextSteps(home, "")
		return nil
	}
	...
	// Render QR code
	_, _ = fmt.Fprintln(cmdStdout)
	_, _ = fmt.Fprintln(cmdStdout, output.Bold("Scan to pair:"))
	qr, err := qrcode.New(pairingURL, qrcode.Medium)
	if err == nil {
		_, _ = fmt.Fprintln(cmdStdout, qr.ToSmallString(false))
	}

	_, _ = fmt.Fprintln(cmdStdout, output.Bold("Pairing link:"))
	_, _ = fmt.Fprintln(cmdStdout, pairingURL)
...
func printNextSteps(home, pairingURL string) {
	_, _ = fmt.Fprintln(cmdStdout)
	_, _ = fmt.Fprintln(cmdStdout, output.Bold("Next steps:"))
	if pairingURL != "" {
		_, _ = fmt.Fprintln(cmdStdout, "  1. Open Solo and scan the QR code above, or paste the pairing link.")
	} else {
		_, _ = fmt.Fprintln(cmdStdout, "  1. Open Solo and connect to your daemon.")
	}
	_, _ = fmt.Fprintln(cmdStdout, "  2. Example: solo run \"your prompt\"")
	_, _ = fmt.Fprintln(cmdStdout, "  3. Docs: https://solo.sh/docs")

	_, _ = fmt.Fprintln(cmdStdout)
	_, _ = fmt.Fprintln(cmdStdout, output.Bold("CLI quick reference:"))
	_, _ = fmt.Fprintln(cmdStdout, "  1. solo --help")
	_, _ = fmt.Fprintln(cmdStdout, "  2. solo ls")
	_, _ = fmt.Fprintln(cmdStdout, "  3. solo run \"your prompt\"")
	_, _ = fmt.Fprintln(cmdStdout, "  4. solo status")
	_, _ = fmt.Fprintf(cmdStdout, "  5. Daemon logs: %s/daemon.log\n", home)

	_, _ = fmt.Fprintln(cmdStdout)
	_, _ = fmt.Fprintln(cmdStdout, output.Green("Solo is ready!"))
}
