...
// CodexAgentClient implements AgentClient for the OpenAI Codex CLI.
type CodexAgentClient struct {
	binaryPath string
	logger     *slog.Logger
}

// NewCodexAgentClient creates a new Codex agent client.
func NewCodexAgentClient(binaryPath string, logger *slog.Logger) *CodexAgentClient {
	if binaryPath == "" {
	...
	return &CodexAgentClient{binaryPath: binaryPath, logger: logger}
}
...
func (c *CodexAgentClient) CreateSession(ctx context.Context, config *protocol.AgentSessionConfig) (AgentSession, error) {
...
func (c *CodexAgentClient) ResumeSession(ctx context.Context, handle *protocol.AgentPersistenceHandle) (AgentSession, error) {
	if err := c.IsAvailable(ctx); err != nil {
		return nil, err
	}
...
func (s *codexSession) StartTurn(ctx context.Context, text string, _ []protocol.ImageAttachment, _ []protocol.AgentAttachment) (<-chan AgentStreamEvent, error) {
	...
	go func() {
		...
		for evt := range baseCh {
			if se, ok := evt.(AgentStreamEvent); ok {
				ch <- se
			}
		}
		close(ch)
...
func (s *codexSession) Subscribe() <-chan AgentStreamEvent {
