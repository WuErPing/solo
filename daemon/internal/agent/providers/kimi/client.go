// Package kimi implements the Kimi Code CLI Wire mode agent provider.
package kimi
...
// Client implements AgentClient for the Kimi Code CLI Wire mode.
type Client struct {
	binaryPath string
	logger     *slog.Logger
}

// NewClient creates a new Kimi agent client.
...
func (c *Client) Provider() string { return kimiProviderName }
...
func (c *Client) ListClientCommands(ctx context.Context, _ string) ([]protocol.AgentSlashCommand, error) {
	if err := c.IsAvailable(ctx); err != nil {
		return nil, err
	}
...
func (c *Client) fetchModelsFromConfig() ([]protocol.AgentModelDefinition, error) {
...
func (s *kimiSession) Run(ctx context.Context, text string, _ []protocol.ImageAttachment, _ []protocol.AgentAttachment, messageID string) (*agent.AgentRunResult, error) {
...
func (s *kimiSession) StartTurn(ctx context.Context, text string, _ []protocol.ImageAttachment, _ []protocol.AgentAttachment) (<-chan agent.AgentStreamEvent, error) {
	runCtx, cancel := context.WithCancel(ctx)
...
func (s *kimiSession) Subscribe() <-chan agent.AgentStreamEvent {
...
func (s *kimiSession) StreamHistory(_ context.Context) ([]agent.AgentStreamEvent, error) {
	return nil, nil
}
