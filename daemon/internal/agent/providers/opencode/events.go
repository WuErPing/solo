...
func (s *openCodeSession) consumeSSE(ctx context.Context, turnID string) (*agent.AgentRunResult, error) {
...
func (s *openCodeSession) finishForegroundTurn(evt agent.AgentStreamEvent, turnID string) {
	s.mu.Lock()
		...
		for callID, item := range s.runningToolCalls {
			notifyEvents = append(notifyEvents, agent.AgentStreamEvent{
				Event: protocol.TimelineStreamEvent{
					Item:     protocol.TimelineItem{Type: "tool_call", CallID: callID, Name: item.Name, Status: "failed", Error: &protocol.ToolError{Message: "Tool execution aborted"}},
					Provider: opencodeProviderName,
				},
				Timestamp: time.Now(),
			})
...
func (s *openCodeSession) translateEvent(eventType string, raw map[string]json.RawMessage) []agent.AgentStreamEvent {
	...
	emit := func(e interface{}) {
		events = append(events, agent.AgentStreamEvent{Event: e, Timestamp: now})
	}
		...
		if props.SessionID == s.base.SessionID() {
			...
			if len(items) > 0 {
				emit(protocol.TimelineStreamEvent{
					Item:     protocol.TimelineItem{Type: "todo", TodoItems: items},
					Provider: opencodeProviderName,
				})
			}
		}
...
func (s *openCodeSession) translateMessagePartDelta(raw map[string]json.RawMessage, emit func(interface{})) {
	...
	if isReasoning {
		...
		emit(protocol.TimelineStreamEvent{
			Item:     protocol.TimelineItem{Type: "reasoning", Text: delta.Delta},
			Provider: opencodeProviderName,
		})
		return
	}
	...
	emit(protocol.TimelineStreamEvent{
		Item:     protocol.TimelineItem{Type: "assistant_message", Text: delta.Delta},
		Provider: opencodeProviderName,
	})
}
...
func (s *openCodeSession) translateMessagePartUpdated(raw map[string]json.RawMessage, emit func(interface{})) {
	...
	case "compaction":
		...
		emit(protocol.TimelineStreamEvent{
			Item:     protocol.TimelineItem{Type: "compaction", CompactionStatus: "loading", Trigger: trigger},
			Provider: opencodeProviderName,
		})
...
func (s *openCodeSession) translatePermissionAsked(raw map[string]json.RawMessage, emit func(interface{})) {
	...
	request := protocol.PermissionRequest{
		...
		Kind:        "tool",
		Title:       base.HumanReadablePermission(props.Permission),
		Input:       input,
		Detail:      detail,
