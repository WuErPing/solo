...
func sendToChannel(ch chan<- agent.AgentStreamEvent, evt agent.AgentStreamEvent) {
...
func (s *openCodeSession) subscribeEvents(cb func(agent.AgentStreamEvent)) func() {
	ch := s.dispatcher.Subscribe()
	done := make(chan struct{})
	go func() {
		defer close(done)
		for evt := range ch {
			cb(evt.(agent.AgentStreamEvent))
		}
	}()
	return func() {
...
func (s *openCodeSession) notifySubscribers(evt agent.AgentStreamEvent) {
	s.dispatcher.Emit(evt)
}

// Subscribe returns a channel that receives all events (implements agent.AgentSession).
func (s *openCodeSession) Subscribe() <-chan agent.AgentStreamEvent {
	src := s.dispatcher.Subscribe()
	ch := make(chan agent.AgentStreamEvent, 256)
	go func() {
		defer close(ch)
		for evt := range src {
			if aevt, ok := evt.(agent.AgentStreamEvent); ok {
...
func (s *openCodeSession) Run(ctx context.Context, text string, images []protocol.ImageAttachment, attachments []protocol.AgentAttachment, messageID string) (*agent.AgentRunResult, error) {
	...
	now := time.Now()
	s.notifySubscribers(agent.AgentStreamEvent{
...
func (s *openCodeSession) StartTurn(ctx context.Context, text string, images []protocol.ImageAttachment, attachments []protocol.AgentAttachment) (<-chan agent.AgentStreamEvent, error) {
	runCtx, cancel := context.WithCancel(ctx)
	turnID := s.createTurnID()
	s.mu.Lock()
	s.base.SetCancelFn(cancel)
	...
	if text != "" {
		s.notifySubscribers(agent.AgentStreamEvent{
			Event: protocol.TimelineStreamEvent{
				Item:     protocol.TimelineItem{Type: "user_message", Text: text},
				Provider: opencodeProviderName,
				TurnID:   turnID,
			},
			Timestamp: time.Now(),
	...
	ch := make(chan agent.AgentStreamEvent, 256)
	unsub := s.subscribeEvents(func(evt agent.AgentStreamEvent) {
...
func (s *openCodeSession) Close() error {
	...
	var notifyEvents []agent.AgentStreamEvent
	for callID, item := range s.runningToolCalls {
		notifyEvents = append(notifyEvents, agent.AgentStreamEvent{
			Event: protocol.TimelineStreamEvent{
				Item:     protocol.TimelineItem{Type: "tool_call", CallID: callID, Name: item.Name, Status: "failed", Error: &protocol.ToolError{Message: "Session closed"}},
				Provider: opencodeProviderName,
			},
			Timestamp: time.Now(),
		})
...
func (s *openCodeSession) StreamHistory(ctx context.Context) ([]agent.AgentStreamEvent, error) {
	var messages []struct {
		Info struct {
			ID         string      `json:"id"`
			...
			for _, part := range msg.Parts {
				...
				case "text":
					if part.Text != "" {
						...
						events = append(events, agent.AgentStreamEvent{
							Event: protocol.TimelineStreamEvent{
								Item:     protocol.TimelineItem{Type: "assistant_message", Text: part.Text},
								Provider: opencodeProviderName,
							},
							Timestamp: time.Now(),
						})
				...
				case "reasoning":
					if part.Text != "" {
						events = append(events, agent.AgentStreamEvent{
							Event: protocol.TimelineStreamEvent{
								Item:     protocol.TimelineItem{Type: "reasoning", Text: part.Text},
								Provider: opencodeProviderName,
							},
							Timestamp: time.Now(),
						})
