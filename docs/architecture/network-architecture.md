...
Solo 是一个 AI 驱动的开发助手平台，采用分层网络架构，支持本地开发和远程协作两种模式。本文档基于实际部署环境编写，详细说明各组件之间的网络路径和数据流。
...
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │           Relay Client                           │   │   │
│  │  │  控制连接 ──► Relay Server (solo.up2ai.top)      │   │   │
│  │  │  数据连接 ◄── Relay Server                       │   │   │
│  │  └─────────────────────────────────────────────────┘   │   │
│  └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
...
```
https://app.solo.sh/#offer={base64url-encoded-json}
```
...
| **App Base URL** | `https://app.solo.sh` | 前端应用地址 |
